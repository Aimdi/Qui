...
void main() {
  group('readFailureKind', () {
    test('classifies transient connectivity failures', () {
      ...
      expect(readFailureKind(http.ClientException('offline')), ReadFailureKind.connection);
    });

    test('classifies account and rate-limit failures', () {
      expect(readFailureKind(NoAccountAvailableException()), ReadFailureKind.session);
      ...
      expect(
        readFailureKind(TwitterError(uri: 'https://x.com', code: 89, message: 'expired')),
        ReadFailureKind.session,
      );
    });

    test('distinguishes endpoint and transaction failures', () {
      expect(readFailureKind(EndpointRefusedException('UserTweets')), ReadFailureKind.endpointRefused);
      expect(
    ...
    test('classifies HTTP failures without exposing response bodies', () {
      expect(readFailureKind(HttpException(http.Response('private body', 503))), ReadFailureKind.serviceUnavailable);
      expect(readFailureKind(HttpException(http.Response('private body', 404))), ReadFailureKind.unavailable);
      expect(readFailureKind(HttpException(http.Response('private body', 429))), ReadFailureKind.rateLimited);
    });
