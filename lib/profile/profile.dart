...
import 'package:qui/profile/_follows.dart';
import 'package:qui/profile/_media_grid.dart';
import 'package:qui/profile/media_grid/media_grid_items/media_grid_item.dart';
import 'package:qui/profile/_saved.dart';
import 'package:qui/profile/_tweets.dart';
...
class _ProfileScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Future<void> retry() => id != null ? model.loadProfileById(id!) : model.loadProfileByScreenName(screenName ?? '');
    Widget waiting(Widget child) => Scaffold(appBar: AppBar(), body: child);
    return ScopedBuilder<ProfileModel, Profile>.transition(
      store: model,
      onError: (_, error) => waiting(
        FullPageErrorWidget(
          error: error,
          stackTrace: null,
      ...
      onLoading: (_) => waiting(const Center(child: CircularProgressIndicator())),
      onState: (_, state) => state.user.idStr == null
          ? waiting(const Center(child: CircularProgressIndicator()))
          : Scaffold(
              body: Column(
                children: [
                  if (state.refreshing) const LinearProgressIndicator(minHeight: 2),
                  if (state.cachedAt != null || state.refreshError != null)
                    Material(
                        ...
                        child: ListTile(
                          ...
                          title: Text(
                            state.refreshError != null
                                ? L10n.of(context).reader_refresh_failed
                                : L10n.of(context).reader_cached_profile,
                          ),
        ...
        children: [
                        ...
                        children: <Widget>[
                          ...
                          Container(
                            ...
                            child: Row(
                              mainAxisAlignment: MainAxisAlignment.end,
                              children: [
                                ProfileFeedSettingsButton(user: user, color: theme.colorScheme.primary),
                                FollowButton(user: UserSubscription.fromUser(user), color: theme.colorScheme.primary),
                                IconButton(
                                  ...
                                  Navigator.push(
                                    ...
                                    MaterialPageRoute(
                                      builder: (context) => TweetMediaView(
                                        ...
                                        media: [
                                          createMediaFromUrl(
                                            user.profileImageUrlHttps?.replaceAll("_normal", "_400x400"),
                                            null,
                                          ),
                                        ],
                                        username: user.screenName ?? "Unknown",
          ...
          AnimatedSwitcher(
            ...
            child: descriptionResized == true && metadataResized == true
                ...
                : Container(key: const Key('waiting'), height: double.infinity, color: theme.colorScheme.surface),
          ),
        ],
      ),
...
class _MediaFilterButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return PopupMenuButton<MediaFilter>(
      ...
      itemBuilder: (context) => [
        for (final filter in MediaFilter.values) PopupMenuItem(value: filter, child: Text(_labelFor(context, filter))),
      ],
      child: Padding(
